...
//    cvtColor( src, image, CV_BGR2Lab );
    ...
    nbrs = Mat( no_of_superpixels,MAX_NBRS, CV_32SC1, Scalar(-1));
    counts_of_nbrs.resize(no_of_superpixels,0);
 ...
 * Get contour separating each clusters
 */
Mat SLICSuperpixel::getNeighbours() {
    ...
    for( int y = 0; y < image.rows; y++ ){
        ...
        for( int x = 0; x < image.cols; x++ ) {
            ...
            for(int k = 0; k < 8; k++ ) {
                ...
                if( withinRange( nx, ny ) )
                    if( clust_ptr[x] != clusters.at<int>(ny, nx) )
                        {
                            int pos = counts_of_nbrs[clust_ptr[x]];
                            int val = clusters.at<int>(ny, nx);

                            bool should_push = true;
                            for(int i=0;i<=pos;i++)
                            {
                                if(nbrs.at<int>(clust_ptr[x],i) == val)
                                {
                                    should_push=false;
                                    break;
                                }
                            ...
                            if(should_push)
                            {
                                nbrs.at<int>(clust_ptr[x],pos) = val;
                                counts_of_nbrs[clust_ptr[x]]++;
                            }
